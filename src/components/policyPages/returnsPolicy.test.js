...
const reducer = {
  returnsPolicy: apiRespone,
};
const store = mockStore({ reducer });

describe("ReturnsPolicy component", () => {
  ...
  const wrapper = shallow(
    <ReturnsPolicy.WrappedComponent store={store} {...props} />
  );

  it("renders ReturnsPolicy components without crashing", () => {
    expect(wrapper).toMatchSnapshot();
  });

  it("test scenarios for componentDidMount function", () => {
    //when APi returns success = false
    wrapper.setProps({ returnsPolicy: { success: false } });
